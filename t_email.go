...
func (p *YookassaPayment) CreatePayment(paymentData map[string]interface{}) (any, error) {
	...
	return payment.Confirmation.(map[string]interface{})["confirmation_url"].(string), err
}

func main() {
	p := NewYookassaPayment("212591", "test_7wZSoyURwPcJA-4iFo5lwseotppiIOu4H7jUMbI9DEA")
	payment, err := p.CreatePayment(map[string]interface{}{})
	if err != nil {
		panic(err)
	}
	fmt.Println(payment)
